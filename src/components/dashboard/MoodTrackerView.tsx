...
export default function MoodTrackerView({ onCheckinComplete, initialTab }: MoodTrackerViewProps) {
  ...
  const [savedLowMoodPrompt, setSavedLowMoodPrompt] = useState<boolean>(false);
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Prompt after submitting low mood */}
      {savedLowMoodPrompt && (
        <div className="p-4 rounded-2xl bg-linear-to-r from-amber-50 to-orange-50 border border-amber-300 flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs shadow-xs animate-in fade-in slide-in-from-top-2">
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="font-bold text-amber-950 text-sm">
                Tu registro se guardó. Cuidemos de ti hoy con calma.
              </p>
              <p className="text-amber-900 text-xs mt-0.5 leading-relaxed">
                Notamos que tu ánimo está en un nivel bajo. Hemos preparado lecturas terapéuticas (David Burns, Marian Rojas), videos y ejercicios de Inteligencia Social para acompañarte sin exigencias ni autojuicio.
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2 self-end sm:self-auto shrink-0">
            <button
              onClick={() => {
                ...
                setSavedLowMoodPrompt(false);
              }}
              className="px-3.5 py-2 bg-stone-900 hover:bg-stone-800 text-white font-semibold rounded-xl text-xs transition-colors flex items-center gap-1.5 shadow-2xs"
            >
              <BookOpen className="w-3.5 h-3.5 text-amber-400" />
              <span>Abrir Recursos de Apoyo</span>
            </button>
            <button
              onClick={() => setSavedLowMoodPrompt(false)}
              className="px-2.5 py-2 text-stone-500 hover:text-stone-800 text-xs font-semibold rounded-xl hover:bg-amber-100/60 transition-colors"
            >
              Cerrar
            </button>
          </div>
